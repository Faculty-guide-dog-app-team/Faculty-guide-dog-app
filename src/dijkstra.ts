...
export class Cell {
    ...
    constructor(distance: number, is_walkable: boolean, coordinates: [number, number], room_text?: string){
 ...
 * @returns
 */
export function number_to_object_grid(number_grid: Array<Array<number>>): Array<Array<Cell>>{
    ...
    for (let row of number_grid){
        ...
        for (let number of row){
            switch (number){ // decyduje czy da się przejść przez komórkę
                case 0: // ścieżka
                ...
                case 1: // ściana
                    ...
                    break;
                case 2: // drzwi
                    object_row.push(new Cell(Infinity, false, [column_index, row_index], "123"));
                    break;
            }
            column_index += 1;
        }
...
// function find_door(room: string, grid: Array<Array<Cell>>): Cell{
//     for (let row of grid){
//         for (let cell of row){
//             if (cell.room_number == room){
//                 return cell
//             }
//         }
//     }
 ...
 * @returns
 */
export function find_path_dijkstra(start: [number, number], goal: [number, number], grid: Array<Array<Cell>>): Array<Cell> {
    let path: Array<Cell> = [];
    ...
    while (unexplored_cells.includes(goal_cell)){
        ...
        let neighbours = [grid[y-1][x], grid[y][x-1], grid[y+1][x], grid[y][x+1]];
        for (let neighbour of neighbours){ // iterating over every neighbour and updating their distances if needed
            if (neighbour.is_walkable || neighbour == goal_cell){ // sprawdza czy da się przejść lub czy jest u celu
    ...
    while (current_cell != start_cell){ // making the array of all parents of the final cell
