...
function turn_coordinates(
  ...
  while (last_cell !== current_cell) {
    ...
    if (difference[0] === 0) {
      // czy nie ma zmiany w X
      if (difference[1] === 1) {
        // czy kolejna jest niżej
        next_facing = 'south';
      } else {
        // skoro nie jest niżej to jest wyżej
        next_facing = 'north';
      }
    } else {
      // zmiana w X
    ...
    if (next_facing !== current_facing) {
      // dołączanie skrętu jeśli zmienia się orientacja
...
function is_passing_nearby_door(
 ...
 */
function doors_passed_in_slice(
  grid: Array<Array<Cell>>,
  path: Array<Cell>,
  previous_turn: [string, Cell],
  next_turn: [string, Cell],
): number {
  ...
  let doors_passed = 0;
  if (start_index === 0) {
    for (let origin_cell of path.slice(start_index, end_index)) {
      // go through every cell in path in range <last turn; next turn)
      if (is_passing_nearby_door(grid, origin_cell, next_facing)) {
        doors_passed += 1;
      }
    }
  } else {
    for (let origin_cell of path.slice(start_index + 1, end_index)) {
      // go through every cell in path in range (last turn; next turn)
      if (is_passing_nearby_door(grid, origin_cell, next_facing)) {
        doors_passed += 1;
      }
