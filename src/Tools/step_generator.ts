...
function translateDirection(
  ...
  return (
    directionMap[current_direction][next_direction.toString()] ||
    'Invalid direction'
  );
}

function translateLastMove(
  current_direction: string,
  next_direction: string,
): string {
  return (
    lastMove[current_direction.toString()][next_direction.toString()] ||
    'Invalid direction'
  );
}
...
export function createInstructions(
  ...
  let if_end_at_turn_2 = [
    path[path.length - 2].coordinates[0] - path[path.length - 3].coordinates[0],
